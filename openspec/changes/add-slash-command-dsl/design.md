...
The existing writer pipeline (`writeAgent`, `writePrompt`) uses an internal `private writeMd` function that accepts a synthetic `Agent` value. `WriterContext` is constructed internally — callers never build it. `Options` is a mutable record configured via a lambda.

Current state: no `Command.fs`, no `writeCommand`, no `SlashCommand` type.

## Goals / Non-Goals

**Goals:**
- Add `SlashCommand` record and `command { }` CE in a new `Command.fs` module.
- Add `MarkdownWriter.writeCommand` that renders `description`-only frontmatter + sections.
...
- JSON/YAML output types for commands — `writeCommand` exposes them for parity but no custom rendering is needed.
...
### 3. Writer delegates to `writeMd` via a synthetic `Agent`

`writeCommand` constructs a synthetic `Agent` with `Frontmatter = Map.ofList ["description", fmStr cmd.Description]` and `Sections = cmd.Sections`, then calls the private `writeMd` (via the same internal path as `writeAgent`). No new rendering logic is needed.

Alternative considered: add a new private `writeCommandMd` function. Rejected — it would duplicate all section/template/import rendering that `writeMd` already handles correctly.
...
`Command.fs` depends on `AST` and `Prompts` (for `Prompt.role` etc.). It must precede `Writers.fs` so `writeCommand` can reference `SlashCommand`. The `.fsproj` compile order: `Tools.fs → AST.fs → Prompt.fs → Agent.fs → Command.fs → Writers.fs → Library.fs`.
...
- **`writeMd` is private** — `writeCommand` must live in the same module (`Writers.fs`) or the synthetic-Agent approach must be used from within the module. Since `writeCommand` is added directly to `MarkdownWriter` in `Writers.fs`, it has access to `writeMd`. No visibility issue.
- **`WriterContext` constructed internally** — `writeCommand` must build its own `ctx` with `AgentName = Some cmd.Name` and `AgentDescription = Some cmd.Description`. This is a one-time copy of the pattern in `writeAgent`; any future refactor to extract context-building should include `writeCommand`.
- **No harness differentiation** — if harnesses diverge in future, `writeCommand` will need the same conditional branching added to `writeMd`. Low risk now; isolated to one function.
