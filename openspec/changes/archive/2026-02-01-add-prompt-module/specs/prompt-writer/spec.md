...
### Requirement: renderPrompt function
The system SHALL provide renderPrompt function in AgentWriter module that accepts Prompt and configuration function.

#### Scenario: Function signature
- **WHEN** user calls AgentWriter.renderPrompt
...
- **WHEN** prompt has Frontmatter with "name" and "description" keys AND renderPrompt is called with Md output type
...
- **WHEN** prompt has role, objective, and instructions sections AND renderPrompt is called
...
- **WHEN** renderPrompt is called with opts.OutputType <- Json
...
- **WHEN** renderPrompt is called with opts.OutputType <- Yaml
...
- **WHEN** opts.HeadingFormatter <- Some (fun s -> s.ToUpper()) AND renderPrompt is called
- **THEN** system SHALL output section headings in uppercase

#### Scenario: Apply rename map to prompt sections
- **WHEN** opts.RenameMap <- Map ["role" -> "Agent Role"] AND renderPrompt is called
...
- **WHEN** prompt Frontmatter has "name" and "description" AND renderPrompt is called
