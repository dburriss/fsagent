...
The `AgentHarness` DU (`Opencode | Copilot | ClaudeCode`) lives inside `AgentWriter` (Writers.fs:41). The `Tool` DU lives in `Tools.fs`. The `renderMd` function's `Template` and `TemplateFile` branches call `Template.renderInline`/`renderFile` directly with `opts.TemplateVariables`.
...
- Build a `toolNameMap` inside `AgentWriter` that maps `Tool` DU case name strings to `Tool` values, then resolves them via the existing harness tool-name logic
- Update `renderMd`'s `Template` and `TemplateFile` branches to use the harness-aware render functions when a harness is present in `opts`
...
**Decision:** Add `renderWithHarness` and `renderFileWithHarness` alongside the existing `renderInline`/`renderFile`. The `renderMd` branches switch to the harness-aware variants; existing callers remain unaffected.
...
**Rationale:** `renderWithHarness` needs `AgentHarness` as a parameter type. F# requires types to be defined before use in the same file. Since both `Template` and `AgentWriter` live in `Writers.fs`, this is the minimal structural change needed.
...
- **`renderMd` harness source** → `renderMd` uses `ctx.Format` (already in scope) as the `AgentHarness`; it is always passed to `renderWithHarness`. There is no optional/None fallback path — the render functions always receive a harness.
