# Spec: Config Paths Module

## Purpose

Provides a public `ConfigPaths` module within `FsAgent.Writers` (re-exported via `open FsAgent`) that exposes harness-specific project and global root resolution logic. Allows consumers to query config root paths without performing file I/O.
